...
from khipu.extended import (
    ...
    peaklist_to_khipu_list,
    read_features_from_text,
)
...
def khipu(
    ...
    delimiter: str = "\t",
    mode: str = "pos",
    mz_tolerance_ppm: int = 5,
    rt_tolerance: int = 2,
    initial_isotopes: list | None = None,
    initial_adducts: list | None = None,
    extended_adducts: list | None = None,
) -> pd.DataFrame:
    """
    Load data from a file using khipu, run khipu, and return a pandas DataFrame.
    """

    if initial_isotopes is None:
        # Isotope patterns for non-tracing HILIC data
        initial_isotopes = isotope_search_patterns[:3]

    if initial_adducts is None:
...
def format_khipu_output(khipu_df: pd.DataFrame) -> pd.DataFrame:
    ...
    khipu_df["mz_group"] = [
        float(i) if pd.notna(i) else khipu_df.iloc[idx]["MZ"] - 1.007825
        for idx, i in enumerate(khipu_df["khipu_id"].str.split("_").str[1])
    ]

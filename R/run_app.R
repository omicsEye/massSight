  ...
  ui <- shiny::fluidPage(
    ...
    shiny::sidebarLayout(
      shiny::sidebarPanel(
        ...
        shiny::selectInput("id_name1", "Compound ID Column:", 
                         choices = c("Select Column" = ""),
                         selected = ""),
        shiny::selectInput("rt_name1", "Retention Time Column:", 
                         choices = c("Select Column" = ""),
                         selected = ""),
        shiny::selectInput("mz_name1", "M/Z Column:", 
                         choices = c("Select Column" = ""),
                         selected = ""),
        shiny::selectInput("int_name1", "Intensity Column:", 
                         choices = c("Select Column" = ""),
                         selected = ""),
        shiny::selectInput("metab_name1", "Metabolite Name Column:", 
                         choices = c("Select Column" = ""),
                         selected = ""),
        ...
        shiny::h5("Column Mapping"),
        shiny::selectInput("id_name2", "Compound ID Column:", 
                         choices = c("Select Column" = ""),
                         selected = ""),
        shiny::selectInput("rt_name2", "Retention Time Column:", 
                         choices = c("Select Column" = ""),
                         selected = ""),
        shiny::selectInput("mz_name2", "M/Z Column:", 
                         choices = c("Select Column" = ""),
                         selected = ""),
        shiny::selectInput("int_name2", "Intensity Column:", 
                         choices = c("Select Column" = ""),
                         selected = ""),
        shiny::selectInput("metab_name2", "Metabolite Name Column:", 
                         choices = c("Select Column" = ""),
                         selected = ""),
        ...
        shiny::actionButton("create_ms", "Create MS Objects",
                          class = "btn-primary",
        ...
        shiny::conditionalPanel(
          ...
          shiny::conditionalPanel(
            ...
            shiny::numericInput("rt_delta", "RT Delta (min):",
                      value = 0.5, min = 0.1, max = 1.0),
            shiny::numericInput("mz_delta", "M/Z Delta (ppm):",
                      value = 15, min = 1, max = 20),
            shiny::numericInput("minimum_intensity", "Minimum Intensity:",
                      value = 1000, min = 0),
            shiny::selectInput("iso_method", "Isolation Method:",
                     choices = c("manual", "dbscan"),
                     selected = "manual"),
            shiny::numericInput("rt_iso_threshold", "RT Isolation Threshold:",
                      value = 0.01, min = 0.001, max = 0.1),
            shiny::numericInput("mz_iso_threshold", "MZ Isolation Threshold:",
                      value = 2, min = 0.1, max = 5)
          ),
          
          # Optimization parameters
          shiny::conditionalPanel(
            condition = "input.optimize",
            shiny::numericInput("n_iter", "Number of Optimization Iterations:",
                      value = 50, min = 10, max = 200)
          ),
          
          # Common parameters
          shiny::selectInput("match_method", "Matching Method:",
                   choices = c("supervised", "unsupervised"),
                   selected = "unsupervised"),
          shiny::selectInput("smooth_method", "Smoothing Method:",
      ...
      shiny::mainPanel(
        width = 9,
        shiny::tabsetPanel(
          shiny::tabPanel("Upload & Parameters", 
                  shiny::h4("Upload your datasets and configure parameters"),
                  shiny::p("Once you've uploaded both datasets and selected the appropriate columns, click 'Create MS Objects' to proceed.")),
          shiny::tabPanel("Data Preview", 
                  DT::DTOutput("dataPreview")),
          shiny::tabPanel("Distribution Plots", 
                  shiny::plotOutput("distPlot1", height = "400px"),
                  shiny::plotOutput("distPlot2", height = "400px")),
          shiny::tabPanel("Alignment Results", 
                  shiny::plotOutput("alignPlot", height = "800px")),
          shiny::tabPanel("Results Summary",
                  shiny::verbatimTextOutput("summary")),
          shiny::tabPanel("Optimization Progress",
                  shiny::conditionalPanel(
                    condition = "input.optimize",
  ...
  server <- function(input, output, session) {
    ...
    observe({
      ...
      tryCatch({
        ...
        updateSelectInput(session, "id_name1", choices = c("Select Column" = "", cols1))
        updateSelectInput(session, "rt_name1", choices = c("Select Column" = "", cols1))
        updateSelectInput(session, "mz_name1", choices = c("Select Column" = "", cols1))
        updateSelectInput(session, "int_name1", choices = c("Select Column" = "", cols1))
        updateSelectInput(session, "metab_name1", choices = c("Select Column" = "", cols1))
    ...
    observe({
      ...
      tryCatch({
        ...
        updateSelectInput(session, "id_name2", choices = c("Select Column" = "", cols2))
        updateSelectInput(session, "rt_name2", choices = c("Select Column" = "", cols2))
        updateSelectInput(session, "mz_name2", choices = c("Select Column" = "", cols2))
        updateSelectInput(session, "int_name2", choices = c("Select Column" = "", cols2))
        updateSelectInput(session, "metab_name2", choices = c("Select Column" = "", cols2))
    ...
    output$distPlot1 <- shiny::renderPlot({
      ...
      validate(
        need(input$file1 && input$file2, "Please upload both datasets first"),
        need(rv$ms_objects_ready, "Please click 'Create MS Objects' button to generate plots")
      )
    ...
    output$summary <- shiny::renderPrint({
      ...
      if (!is.null(rv$opt_history)) {
        ...
        cat("Final score:", round(rv$opt_history$final_score, 4), "\n")
        cat("\nOptimal parameters:\n")

...
We'll also need to install the package `omicsArt`. This package provides helper functions for loading, wrangling, and visualizing data. Instructions for installing `omicsArt` can be found [here](https://github.com/omicsEye/omicsArt).
...
# library(omicsArt)
library(kableExtra)
# iHMP_QI <- system.file("data", "HMP2_metabolomics.xlsx", package = "massSight")
# iHMP_CD <- system.file("data", "iHMP_C18-FFA_CompoundDixcoverer_Exported_All_Samples_Annotated.csv", package = "massSight")
```
...
```{r, eval = F, cache = T, results = 'hide', message=FALSE, warning=FALSE}
loaded_data <-
  massSight::load_data(
...
```{r, eval = F}
C18_CD <- read.delim(
  "data/C18neg_FFA method/iHMP_C18-FFA_CompoundDiscoverer_Exported_All_Samples_Annotated.csv",
  sep = ",",
  header = TRUE,
  fill = FALSE,
  comment.char = "",
